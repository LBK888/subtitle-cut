...
function cacheDom() {
  ...
  dom.manuscriptRestoreAll = document.querySelector("#manuscript-restore-all");
...
function handleTranscriptMouseUp(event) {
  ...
  const additive = true;
  toggleSelection(keys, { additive, mode: "add" });
...
async function handleUploadAndTranscribe(event) {
  ...
  try {
    ...
    const model = dom.modelSelect?.value || "Qwen/Qwen3-ASR-1.7B";
    const language = document.querySelector("#language-select")?.value || "auto";
...
function handleEngineChange() {
  ...
  // BEGIN-EDIT
  const engine = dom.engineSelect.value || "qwen3-asr";
  const options = ENGINE_MODEL_OPTIONS[engine] || ENGINE_MODEL_OPTIONS["qwen3-asr"];
  // END-EDIT
