...
def download_vad(vad_path: Path, progress_cb=None):
    """下載 Silero VAD 模型。"""
    if _file_is_real(vad_path):
        if progress_cb:
            progress_cb(1.0, "✅ VAD 模型已存在")
        return
    ...
    def _file_cb(done: int, total: int):
        if progress_cb and total > 0:
            progress_cb(done / total, f"下載 VAD… {done/1_048_576:.1f} / {total/1_048_576:.1f} MB")

    _download_file(_VAD_URL, vad_path, progress_cb=_file_cb)
    
    if progress_cb:
        progress_cb(1.0, "✅ VAD 模型下載完成！")
