    ...
    def load(self, device: str = "cuda", model_dir: Path = None,
        ...
        if vad_path is None:
            try:
                import downloader
                _s("自動下載 VAD 模型…")
                vad_dest = OV_MODEL_DIR / "silero_vad_v4.onnx"
                downloader.download_vad(vad_dest)
                vad_path = vad_dest
        ...
        try:
            ...
            import downloader
            if not downloader.quick_check_diarization(OV_MODEL_DIR):
                _s("自動下載說話者分離模型…")
                downloader.download_diarization(OV_MODEL_DIR / "diarization")
            
            diar_dir = OV_MODEL_DIR / "diarization"
            eng = DiarizationEngine(diar_dir)
            self.diar_engine = eng if eng.ready else None
        except Exception as e:
            _s(f"說話者分離模型載入或下載失敗: {e}")
            self.diar_engine = None

        asr_path_str = str(asr_path) if asr_path.exists() else "Qwen/Qwen3-ASR-1.7B"
        aligner_path_str = str(aligner_path) if aligner_path.exists() else "Qwen/Qwen3-ForcedAligner-0.6B"

        # ── PyTorch ASR 模型 ──────────────────────────────────────────
        _s(f"載入 ASR 模型（{asr_path_str}）…")
        ...
        if use_aligner:
            try:
                _s(f"載入時間軸對齊模型（{aligner_path_str}）…")
...
class App(ctk.CTk):
    ...
    def _startup_check(self):
        ...
        # Relaxed check: we allow fallback to HF hub now
